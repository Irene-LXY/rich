...
> 【2.0变更】 STEP的steps允许0以及大于70。steps必须是0..2147483647的整数；steps=0时玩家不移动并按当前位置执行一次落点处理；当steps>70时，先计算effective_steps=steps%70，再按effective_steps执行移动。示例：0→原地结算，69→69步，70→绕行70步回到原地，71→1步，140→原地结算。

负数、小数、字符串、null或缺失仍返回INVALID_PARAMS。STEP 0是合法游戏Action，会执行当前位置的落点处理并在交互完成后结束回合；ADVANCE_TURN只推进回合和计时器，不触发移动或落点，两者不得混用。

### 6.1 道具屋购买与退出

> 【2.0补充】 道具屋购买成功后先扣点并增加道具，再根据购买后的状态决定是否退出。

- 购买后剩余点数低于最便宜道具的30点时，自动退出道具屋并在交互完成后结束当前回合；即使该次购买刚好使道具总数达到10，也按此规则自动退出。
- 购买后剩余点数不少于30点时，继续保持`phase=PROMPT`、`pending_prompt=TOOL_SHOP`和当前玩家不变，可继续选择。
- 道具总数达到10且剩余点数不少于30时不自动退出；再次购买返回购买失败，道具和点数不变，并继续等待玩家输入`F`。
- 输入`F`手动退出道具屋，清空Prompt并结束当前回合。
...
| STEP | {steps:0..2147483647} | 0表示原地结算；steps>70时按地图70格循环计算落点 | 是（Prompt完成后） | 变更 |
...
| STEP 0推进时间 | 改为合法原地结算 | 会触发当前位置落点；仅推进计时且不触发落点时使用ADVANCE_TURN |
| STEP最大69步 | 取消上限 | 允许非负int32；steps>70时按地图70格循环计算落点 |
