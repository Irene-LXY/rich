...
CommandResult command_execute(
    ...
    if (game == 0 || input == 0 || strlen(input) >= sizeof(buffer)) {
        write_message(message, message_size, u8"命令无效。\n");
        return COMMAND_INVALID;
    }
    if (game->phase == GAME_ENDED) {
        write_message(message, message_size, u8"游戏已经结束，后续命令全部无效。\n");
    ...
    text = trim(buffer);
    if (*text == '\0') {
        write_message(message, message_size, u8"命令不能为空。\n");
        return COMMAND_INVALID;
    }
    ...
    write_message(message, message_size, u8"无效或尚未实现的命令。\n");
    return COMMAND_INVALID;
}
