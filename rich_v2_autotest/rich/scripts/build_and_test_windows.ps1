...
#      - 有 gcc/clang 且有 mingw32-make -> MinGW Makefiles
#        （在中文真实路径的 junction 场景下比 Ninja 更稳定）
#      - 没有 MinGW Makefiles 条件但有 Ninja -> Ninja
...
if ($Generator) {
    $genArgs = @('-G', $Generator)
    $genLabel = $Generator
} elseif (((Get-Command gcc -ErrorAction SilentlyContinue) -or
...
# 生成器变化时清理旧构建目录，避免 CMake generator mismatch，
# 也避免 Ninja 失败后留下的 depfile 干扰后续 MinGW Makefiles 构建。
...
if ($chosenGenName -and (Test-Path -LiteralPath $cacheFile)) {
    ...
    if ($cachedGen) {
        ...
        if ($cachedGenName -and $cachedGenName -ne $chosenGenName) {
            Write-Host "[提示] 旧缓存生成器 '$cachedGenName' 与本次 '$chosenGenName' 不一致，清理构建目录。" -ForegroundColor Cyan
