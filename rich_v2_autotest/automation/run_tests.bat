...
echo.
echo ===== STATE positive cases: 258 =====
echo Program: %STATE_TARGET%
"%STATE_RUNNER%" --program "%STATE_TARGET%" --cases "%~dp0testcases\FirstGroup_Iteration3_State_v2.json" --map "%~dp0spec\map.json" --out "%~dp0results_state_v2.json" --junit "%~dp0junit_state_v2.xml"
...
if "%STATE_EXIT%"=="0" if "%CLI_EXIT%"=="0" (
    echo All 288 iteration 3 positive cases passed.
    set "FINAL_EXIT=0"
    goto :finish
)
