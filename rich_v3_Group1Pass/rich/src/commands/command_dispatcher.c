...
CommandResult command_execute(
    ...
    /* 礼品屋、道具屋和房产选择属于当前落地事件，不作为普通命令解析。 */
    ...
    if (equals_ignore_case(text, "step")) {
        int steps;
        if (!parse_nonnegative_int(arguments, &steps)) {
            write_message(message, message_size,
                          "Step 命令格式为 Step n，n 必须是非负整数。\n");
            return COMMAND_INVALID;
        }
    ...
    if (equals_ignore_case(text, "block")) {
        char *end;
        long parsed = strtol(arguments, &end, 10);
        if (end == arguments || *end != '\0' || parsed < -10 || parsed > 10) {
            write_message(message, message_size,
                          "用法：Block n，n 范围为 -10~10。\n");
            return COMMAND_INVALID;
        }
        if (runtime_place_tool(game->runtime, 1, (int)parsed,
