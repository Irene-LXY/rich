...
static int build_preset(const JValue *case_obj, AutomationPreset *p, char *err, size_t errsz) {
    ...
    p->turn_number = (uint64_t)jv_int(jv_get(preset, "turn_number"), 1);
    if (p->turn_number == 0U) {
        seterr(err, errsz, "invalid turn_number");
        return -1;
    }
    ...
    fortune = jv_get(preset, "fortune");
    if (fortune != NULL && fortune->type == J_OBJ) {
        const JValue *position = jv_get(fortune, "position");
        const JValue *next_spawn = jv_get(fortune, "next_spawn_after_turn");
        p->fortune_configured = 1;
        if (position != NULL && position->type == J_INT) {
            p->fortune_position = (int)position->i;
            if (p->fortune_position < 0 || p->fortune_position >= RICH_MAP_SIZE) {
                seterr(err, errsz, "invalid fortune position");
                return -1;
            }
        }
        p->fortune_remaining_map_turns =
            (int)jv_int(jv_get(fortune, "remaining_map_turns"), 0);
        if (p->fortune_remaining_map_turns < 0 ||
            p->fortune_remaining_map_turns > 5) {
            seterr(err, errsz, "invalid fortune remaining turns");
            return -1;
        }
        if (next_spawn != NULL && next_spawn->type == J_INT) {
            if (next_spawn->i < 0) {
                seterr(err, errsz, "invalid fortune next spawn turn");
                return -1;
            }
            p->fortune_next_spawn_turn = (uint64_t)next_spawn->i + 1U;
        }
    }
...
static int execute_actions(GameRuntime *rt, const JValue *actions,
    ...
    for (ai = 0; ai < actions->count; ++ai) {
        ...
        } else if (cmd_equals_ignore_case(cmd, "ADVANCE_TURN")) {
            if (runtime_advance_turn_for_test(rt, msg, sizeof(msg)) != 0) {
                seterr(err, errsz, "advance turn failed");
                return -1;
            }
