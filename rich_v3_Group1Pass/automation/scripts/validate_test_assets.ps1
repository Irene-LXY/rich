...
$caseFiles = if (Test-Path -LiteralPath $caseDir) {
    @(Get-ChildItem -LiteralPath $caseDir -Filter '*.json' -File -Recurse | Sort-Object FullName)
} else {
    @()
}
...
$stateCasesList = [System.Collections.Generic.List[object]]::new()
foreach ($caseFile in $caseFiles) {
    $caseDocument = Read-JsonFile $caseFile.FullName
    if ($null -eq $caseDocument) { continue }
    if ([string]$caseDocument.schema_version -ne '2.0') {
        Add-Failure "$($caseFile.FullName) 的 schema_version 必须为2.0。"
    }
    if ($caseDocument.PSObject.Properties.Name -contains 'tests') {
        $documentCases = @($caseDocument.tests)
        if ($documentCases.Count -eq 0) { Add-Failure "$($caseFile.FullName) 的 tests 为空。" }
    } elseif ($caseDocument.PSObject.Properties.Name -contains 'case_id') {
        $documentCases = @($caseDocument)
    } else {
        Add-Failure "$($caseFile.FullName) 既不是单条用例，也不包含 tests 数组。"
        $documentCases = @()
    }
    foreach ($documentCase in $documentCases) { $stateCasesList.Add($documentCase) }
}
$stateCases = @($stateCasesList)
...
if ($caseFiles.Count -gt 0 -and $stateCases.Count -eq 0) { Add-Failure 'testcases 下没有可执行用例。' }
...
foreach ($case in $stateCases) {
    $id = [string]$case.case_id
    if ([string]::IsNullOrWhiteSpace($id)) { Add-Failure 'testcases 中存在空case_id。'; continue }
    if (-not $stateIds.Add($id)) { Add-Failure "STATE case_id重复: $id" }
    if ([string]$case.schema_version -ne '2.0') { Add-Failure "$id 未显式声明schema_version=2.0。" }
    if ([string]$case.mode -ne 'STATE') { Add-Failure "$id 在STATE活动文件中mode不是STATE。" }
    if ($null -eq $case.preset -or $null -eq $case.actions -or $null -eq $case.expected) {
        Add-Failure "$id 缺少preset、actions或expected。"
    }
    if ($null -ne $case.expected_outcome -or $null -ne $case.expected_error -or
        $null -ne $case.expected_errors -or $null -ne $case.expected_error_code -or
        $null -ne $case.expected_result) {
        Add-Failure "$id 使用了已删除的负向用例字段。"
    }
    $stateSourceText = $case.source_trace | ConvertTo-Json -Depth 10 -Compress
    if ($stateSourceText -match '测试类型.{0,3}(反向|负向)|非法输入|错误命令|启动失败') {
        Add-Failure "$id 的原始说明明确标注为负向场景。"
    }
    $statePhase = [string]$case.expected.phase
    $statePrompt = $case.expected.pending_prompt
    $allowedStatePrompts = @('BUY', 'UPGRADE', 'TOOL_SHOP', 'GIFT_SHOP')
    if ($null -ne $statePrompt -and $statePrompt -isnot [string]) {
        Add-Failure "$id 的expected.pending_prompt必须是null或字符串枚举。"
    } elseif ($statePhase -eq 'PROMPT' -and ([string]::IsNullOrWhiteSpace([string]$statePrompt) -or [string]$statePrompt -notin $allowedStatePrompts)) {
        Add-Failure "$id 的expected.phase为PROMPT时必须给出合法pending_prompt。"
...
Write-Host "[通过] v2.0测试资产校验完成：已扫描$($caseFiles.Count)个STATE JSON文件、$($stateCases.Count)条STATE用例和$($interactiveFiles.Count)条INTERACTIVE用例。" -ForegroundColor Green
exit 0
