...
#   .\build.ps1 -NoPause         # 不暂停（供工程根build.bat和run_tests.bat内部调用）
...
Write-Host "[3/3] 校验迭代三地图与 JSON 用例 ..." -ForegroundColor Cyan
& (Join-Path $PSScriptRoot 'validate_test_assets.ps1') -ProjectRoot $projectRoot
if ($LASTEXITCODE -ne 0) {
    Write-Host '[错误] 测试资产校验失败。' -ForegroundColor Red
    Invoke-Pause
    exit $LASTEXITCODE
}
