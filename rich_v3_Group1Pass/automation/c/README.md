...
1. 读取测试用例 JSON；
2. 调用开发人员提供的程序；
3. 按 `../spec/game_test_spec_v2.md` 做 Expected 部分匹配判定；
...
- `--timeout-ms <n>`：单条 STATE 适配器超时，范围100～300000毫秒，默认10000。
- `--quiet`：只输出汇总。
...
- `fields_absent`：可写在任意 Expected 对象中，列出的直接子字段不得存在。
- `fortune_assert`：迭代三随机财神虚拟断言，支持 `present`、`position_between`、
  `position_not_in`、`unoccupied`、`without_map_item`。
...
测试用例 JSON 结构、迭代三财神 Preset/Actual 扩展、`map.json` 结构，以及 70 格
权威地图布局，请见 `../README.md`、`../spec/game_test_spec_v2.md` 和
`../spec/map.json`。`iteration3_contract.md` 只保留旧链接兼容说明，不再是独立规范。
...
6. Windows 下按系统 ANSI 代码页处理命令行路径（中文版 Windows 为 GBK），
